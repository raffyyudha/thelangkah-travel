...
export default function AdminDashboard() {
  ...
  const tours = [
    { id: "whale-shark-1day-labuhan-jambu", name: "Tour: Whale Shark 1 Day Labuhan Jambu" },
    { id: "whale-shark-2d1n", name: "Tour: Whale Shark 2D1N (Start Sumbawa)" },
    { id: "whale-shark-speedboat", name: "Tour: Whale Shark Speedboat" },
    { id: "combo-moyo-whale-shark", name: "Tour: Combo Moyo & Whale Shark" },
    { id: "trip-4d3n-sumbawa", name: "Tour: Trip 4D3N Sumbawa" },
    { id: "transport-service", name: "Service: Transport (Pick-up & Drop-off)" },
  ];
  ...
  const loadDefaultTransportPrices = async (skipConfirm = false) => {
    if (!skipConfirm && !confirm('Load default prices for Transport Service? This will add 4 default routes.')) return;
    
    const defaults = [
      {
        tour_name: 'transport-service',
        participants: 'Bandara atau Hotel Sumbawa → Labuhan Jambu',
        full_private_price: 'Rp 600.000',
        open_trip_price: '1–5 orang'
      },
      {
        tour_name: 'transport-service',
        participants: 'Pelabuhan Poto Tano → Labuhan Jambu',
        full_private_price: 'Rp 1.200.000',
        open_trip_price: '1–5 orang'
      },
      {
        tour_name: 'transport-service',
        participants: 'Pelabuhan Sape → Labuhan Jambu',
        full_private_price: 'Rp 1.500.000',
        open_trip_price: '1–5 orang'
      },
      {
        tour_name: 'transport-service',
        participants: 'Hotel Maluk → Labuhan Jambu',
        full_private_price: 'Rp 1.700.000',
        open_trip_price: '1–5 orang'
      }
    ];

    const { error } = await supabase.from('tour_prices').insert(defaults);
    if (error) alert('Error adding defaults: ' + error.message);
    else {
      if (!skipConfirm) alert('Default prices loaded successfully!');
      loadPrices();
    }
  };
  ...
  // Auto-seed transport prices if missing
  useEffect(() => {
    if (selectedTour === 'transport-service' && prices.length > 0) {
      const transportPrices = prices.filter((p: TourPrice) => p.tour_name === 'transport-service');
      if (transportPrices.length === 0) {
        console.log("Auto-seeding default transport prices...");
        loadDefaultTransportPrices(true);
      }
    }
      ...
      <div className="max-w-7xl mx-auto">
        ...
        <div className="bg-white rounded-lg shadow-md p-4 md:p-6 mb-6">
          ...
          <div className="flex gap-2 flex-wrap">
            ...
            <Link href="/admin/articles" className="bg-orange-500 text-white px-4 py-2 rounded-lg hover:bg-orange-600 text-sm">
              📝 Manage Articles
            </Link>
          </div>
        </div>
        ...
        <div className="bg-white rounded-lg shadow-md p-4 md:p-6 mb-6">
          <div className="flex justify-between items-center mb-6">
            ...
            <div className="flex gap-2">
              {selectedTour === 'transport-service' && (
                <button 
                  onClick={() => loadDefaultTransportPrices()}
                  className="bg-blue-100 text-blue-700 px-3 md:px-4 py-2 rounded-lg hover:bg-blue-200 text-sm md:text-base font-semibold"
                >
                  📥 Load Default Prices
                </button>
              )}
        ...
        {editingPrice && (
          <div className="bg-gray-50 p-4 rounded-lg mb-6">
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
              <input
                type="text"
                placeholder={selectedTour === 'transport-service' ? "Route / Tujuan (e.g. Bandara -> Labuhan Jambu)" : "Participants (e.g., 2-10)"}
              ...
              <input
                type="text"
                placeholder={selectedTour === 'transport-service' ? "Capacity / Kapasitas (e.g. 1-5 orang)" : "Open Trip Price"}
              ...
              <input
                type="text"
                placeholder={selectedTour === 'transport-service' ? "Price / Harga (e.g. Rp 600.000)" : "Full Private Price"}
          ...
          <table className="w-full">
            <thead className="bg-gray-100">
              <tr>
                <th className="px-4 py-2 text-left text-sm">
                  {selectedTour === 'transport-service' ? "Route / Tujuan" : "Participants"}
                </th>
                <th className="px-4 py-2 text-left text-sm">
                  {selectedTour === 'transport-service' ? "Capacity / Kapasitas" : "Open Trip"}
                </th>
                <th className="px-4 py-2 text-left text-sm">
                  {selectedTour === 'transport-service' ? "Price / Harga" : "Full Private"}
                </th>
            ...
            <tbody>
              ...
              {currentTourPrices.length === 0 && (
                <tr><td colSpan={4} className="px-4 py-8 text-center text-gray-500">No prices yet.</td></tr>
              )}
            </tbody>
          </table>
        </div>
      ...
      <div className="bg-white rounded-lg shadow-md p-4 md:p-6">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-6 mb-6">
          {['hero', 'gallery1', 'gallery2', 'gallery3'].map((type) => {
            ...
            // Fallback logic for transport-service
            let displayImage = existingImage ? existingImage.image_url : null;
            if (!displayImage && selectedTour === 'transport-service') {
              if (type === 'hero') displayImage = '/images/transport-1.webp';
              else if (type === 'gallery1') displayImage = '/images/transport-2.webp';
              else if (type === 'gallery2') displayImage = '/images/transport-3.webp';
              else if (type === 'gallery3') displayImage = '/images/transport-4.webp';
            }
                ...
                {displayImage ? (
                  <div>
                    <div className="relative h-64 mb-3 rounded-lg overflow-hidden border-2 border-gray-300">
                      <Image src={displayImage} alt={type} fill className="object-cover" unoptimized={displayImage.startsWith('http')} />
                    </div>
                    <div className="flex gap-2">
                      <label className="flex-1 bg-blue-500 text-white px-4 py-2 rounded-lg hover:bg-blue-600 cursor-pointer text-center text-sm font-semibold">
                        {existingImage ? "Replace Image" : "Upload Custom Image"}
                        <input
                          type="file"
                          accept="image/*"
